...
import { Firestore, collection, setDoc, doc, query } from '@angular/fire/firestore';  // Métodos para interactuar con Firestore
import { CategoriaI } from '../models/categoria.models';
import { getDocs, where } from 'firebase/firestore';  // Métodos para hacer lecturas y filtros en Firestore
import { PosturaI } from '../models/postura.models';

@Injectable({
    providedIn: 'root' // Para que el servicio esté disponible en toda la aplicación
})
export class CategoriasService {
    constructor(private firestore: Firestore) { } // Inyectamos Firestore en el constructor para poder usarlo

    async getCategorias(): Promise<CategoriaI[]> {
        const coleccionCategorias = collection(this.firestore, 'categorias'); //Obtenemos la referencia a la colección de categorías
        ...
        } as CategoriaI)); //Mapeamos los documentos a un array de CategoríaI
        return categorias; //Retornamos el array de categorías
    }


    async addCategorias() {
        //Declaramos un array de categorías con sus respectivos beneficios, contraindicaciones y ejemplos de posturas
        ...
        const categoriasCollection = collection(this.firestore, 'categorias'); // Obtenemos la referencia a la colección de categorías

        try {
            
            const categoriasExistentes = await this.getCategorias();  //Llamamos a nuestro otro método para que nos de las categorías existentes
            ...
            for (const categoria of categorias) { //Vamos recorriendo el array de categorías
                // Vemos si la categoría ya existe en la base de datos comparando los idsExistentes con el id de la categoría que estamos iterando
                if (!idsExistentes.includes(categoria.id)) { // Si no existe, la agregamos a la base de datos
                    ...
                    await setDoc(newDocRef, {
                        id: categoria.id, //Aquí también podría llamar a mi otro servicio 
    ...
    async getPosturasDeCategoria(categoriaId: string): Promise<PosturaI[]> {
        ...
        // Conviertimos el string de categoriaId a número
        const categoriaIdNumero = Number(categoriaId);

        const coleccionPosturas = collection(this.firestore, 'posturas'); //Obtenemos referencia a la colección de posturas
        // Como cada postura tine un categoria_id podemos hacer la consulta por ese campo, categoria_id es un número en nuestro modelo postura y en Firebase
        const consultaQuery = query(coleccionPosturas, where('categoria_id', '==', categoriaIdNumero));

        const instantanea = await getDocs(consultaQuery); // Realizamos la consulta que nos da una instantanea con los documentos que coinciden

        const posturasPorCategoria: PosturaI[] = []; //Array de posturas vacío para luego guardar

        for (const documentoInstantanea of instantanea.docs) { // Recorremos todos los documentos de la instantanea
            const data = documentoInstantanea.data() as PosturaI; //Obtengo los datos del documento y los casteo como PosturaI
            posturasPorCategoria.push(data); // Añadimos al array de posturas con push 
        }
     ...
     //Método que busca todas las categorias. Retorna una promesa que se resuelve con un array de CategoriaI.
    async getTodasCategorias(): Promise<CategoriaI[]> {
        ...
        const consultaQuery = query(coleccionCategorias); 
        ...
        const categorias: CategoriaI[] = []; //Array de categorias vacío para luego guardar

        for (const documentoInstantanea of instantanea.docs) { //Vamos recorriendo todos los documentos de la instantanea 
            const data = documentoInstantanea.data() as CategoriaI; //Obtengo los datos del documento y casteamos como CategoriaI
